...
def test_license_data(db, test_user):
    ...
    Returns a dict with:
    - default_license: A default LicenseKey object (the user's last license)
    - autogenerated_licenses: List of existing autogenerated licenses (if any)
    
    Note: The default_license must be created LAST so that .last() returns it.
    """
    # Create default organization if needed
    default_org, _ = Organization.objects.get_or_create(
        slug='default',
    ...
    # These must be created BEFORE the default license
    ...
    # Create the default license LAST so that .last() returns it
