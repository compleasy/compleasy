...
def settings_view(request):
    ...
    if request.method == 'POST':
        ...
        package_formset = EnrollmentPackageFormSet(
            request.POST,
            instance=settings_instance,
            prefix='package',
        )
        ...
        if (
            form.is_valid()
            and plugin_formset.is_valid()
            and package_formset.is_valid()
            and skip_formset.is_valid()
        ):
...
def onboarding(request):
    ...
    import json
    # Use TRIKUSEC_LYNIS_API_URL for API operations (upload-server and enrollment script)
    trikusec_lynis_api_url = settings.TRIKUSEC_LYNIS_API_URL

    # Get only the host:port from the API URL
    parsed_lynis_api_url = urlparse(trikusec_lynis_api_url)
    trikusec_lynis_upload_server =  parsed_lynis_api_url.netloc + '/api/lynis'
    trikusec_lynis_enroll_url = f'{parsed_lynis_api_url.scheme}://{parsed_lynis_api_url.netloc}/api/lynis/enroll/'
    ...
    return render(request, 'onboarding.html', {
        'trikusec_lynis_enroll_url': trikusec_lynis_enroll_url,
