...
function initLicenseSelection(config) {
    const {
        trikusecLynisEnrollUrl,
        trikusecLynisUploadServer,
        defaultLicenseKey,
    ...
    // Use TRIKUSEC_LYNIS_API_URL for downloads (enroll.sh)
    // trikusecLynisUploadServer is already in format host:port/api/lynis (from backend)
    
    // Use selected license key if provided, otherwise use default
    let currentLicenseKey = selectedLicenseKey || defaultLicenseKey;
    const buildEnrollCommand = (licenseKey) => `wget -qO- --no-check-certificate ${trikusecLynisEnrollUrl}?licensekey=${licenseKey} | bash`;
    let enrollCommand = buildEnrollCommand(currentLicenseKey);
    let manualInstallConfig = `license-key=${currentLicenseKey}\nupload-server=${trikusecLynisUploadServer}`;
    
    // Function to update the enroll command
    function updateEnrollCommand(licenseKey) {
        currentLicenseKey = licenseKey;
        enrollCommand = buildEnrollCommand(licenseKey);
