...
TrikuSec follows a **read-only security model** to ensure maximum protection for your infrastructure:

- **Read-Only Operations**: TrikuSec only **reads** audit data from your servers. It never pushes changes, executes commands, or modifies configurations on monitored servers.
- **Passive Monitoring**: All data flows one-way: from your servers to TrikuSec. Your servers remain completely autonomous and secure.
...
This approach ensures that TrikuSec enhances your security posture without introducing new attack vectors or dependencies on your production servers.
...
3. Run Lynis with the custom profile and upload the results to the TrikuSec server:
...
1. Access the TrikuSec server at `http://yourserver:3000`.

### License Management

TrikuSec supports flexible license key management with two models:
...
TrikuSec uses SQLite by default for easy installation, but PostgreSQL is **strongly recommended** for production deployments due to better performance, concurrency, and reliability.
...
Then set the `DATABASE_URL` environment variable in your TrikuSec service:
...
- Never use default admin credentials in production. Set `TRIKUSEC_ADMIN_PASSWORD` in your `.env`.
...
```bash
docker compose -f docker-compose.dev.yml exec trikusec-dev python manage.py shell <<EOF
from django.contrib.auth.models import User
from api.models import LicenseKey
user = User.objects.first()
LicenseKey.objects.create(licensekey='$TRIKUSEC_LICENSE_KEY', created_by=user)
EOF
```
...
- `src/trikusec/` - Django project settings
  - `src/trikusec/settings/` - Environment-specific settings package
...
TrikuSec supports API versioning while maintaining backward compatibility:
...
- `docker compose.dev.yml` - Development environment with TrikuSec and Lynis client
